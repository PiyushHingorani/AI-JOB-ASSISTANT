...
from utils import extract_raw_text_from_pdf, create_formatted_cover_letter_docx, clean_text
...
def create_streamlit_app(llm):
    ...
    with col1:
        ...
        manual_job_desc = st.text_area("Or paste the job description here (if link fails):", height=200)
        ...
        if submit_button:
            try:
                if manual_job_desc.strip():
                    cleaned_job_data = clean_text(manual_job_desc)
                elif "job_url" in st.session_state:
                    loader = WebBaseLoader([st.session_state["job_url"]])
                    job_data = loader.load().pop().page_content
                    cleaned_job_data = clean_text(job_data)
                else:
                    st.error("Please provide a Job URL or paste a job description.")
                    return

                jobs = llm.extract_jobs(cleaned_job_data)
                # Filter out invalid jobs
                jobs = [job for job in jobs if isinstance(job, dict) and job.get('role')]
                if not jobs:
                    st.error("No valid jobs found in the extracted data. The page may not contain job listings or the format is not supported.")
                else:
        ...
        if st.button("Quick Cover Letter Generator"):
            if "resume_info" in st.session_state and "job_data" in st.session_state:
                ...
                for idx, job in enumerate(jobs):
                    role = job.get('role', f"unknown_{idx}")
                    if not job.get('role'):
                        st.warning(f"Job entry missing 'role' key: {job}")
                    cover_letter = llm.write_cover_letter(job, resume_info)
                    st.session_state[f"cover_letter_text_{role}"] = cover_letter

                st.session_state["generate_cover_letter"] = True
            else:
                st.error("Both resume and Job link or description required to generate cover letter")
    ...
    with col2:
        ...
        if st.session_state.get("generate_cover_letter", False):
            ...
            for idx, job in enumerate(jobs):
                ...
                st.markdown(f"### Cover Letter for {role} at {company}")

                # Editable text box for the generated cover letter
                cover_letter_textarea = st.text_area(
                    "Edit your cover letter here:",
                    value=st.session_state.get(f"cover_letter_text_{role}", ""),
                    height=400,
                    key=f"cover_letter_textarea_{role}"
                )

                # Update session state with the latest edited cover letter
                st.session_state[f"cover_letter_text_{role}"] = cover_letter_textarea

                # Generate and automatically download .docx when clicking the button
                if st.button(f"Download Cover Letter (.docx)", key=f"download_{role}"):
                    try:
                        # Use the updated text from session state
                        docx_file_path = create_formatted_cover_letter_docx(st.session_state[f"cover_letter_text_{role}"])
                        ...
                        with open(docx_file_path, "rb") as docx_file:
                            ...
                            href = f'<a href="data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,{b64}" download="cover_letter_{role}.docx">Click here if download not started yet</a>'
